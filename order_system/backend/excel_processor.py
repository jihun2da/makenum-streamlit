        ...
        for row in raw_rows:
            try:
                ...
                if existing_item:
                    ...
                    change_note = f"[{now_str}] 재업로드: {', '.join(changes)}" if changes else f"[{now_str}] 재업로드 (변경없음)"
                    patch["change_log"] = (item.get("change_log") or "") + "\n" + change_note if item.get("change_log") else change_note

                    if patch:
                        supabase.table("order_items").update(patch).eq("id", item["id"]).execute()

                    # 재업로드 activity_log 기록
                    supabase.table("activity_log").insert({
                        ...
                        "new_value":        row["status"] if changes else item["status"],
                        "note":             change_note,
                        "upload_history_id": upload_id,
                    }).execute()
                else:
